...
@interface QMQColumnListViewController ()<UITableViewDelegate, UITableViewDataSource>

@property(nonatomic, strong) UITableView            *tableView;
@property(nonatomic, strong) QMQColumnListViewModel *viewModel;

@end

@implementation QMQColumnListViewController
...
- (void)viewDidLoad {
    ...
    @weakify(self);
    [self.getColumnDetailSignal subscribeNext:^(QMQColumnModel *model) {
        @strongify(self);
        self.title = model.name;
        [self initViewModel];
        [self bindViewModel];
        [self loadData:model.columnId];
    }];
}
...
- (void)initViewModel {
    _viewModel = [[QMQColumnListViewModel alloc] init];
